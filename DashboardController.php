...
where tgl_input BETWEEN '$tgl_dari_default' and '$tgl_sampai_default' group by kode_satker"));
        ...
        //echo "<pre>"; print_r($satker_id); die;
...
where tgl_input BETWEEN '$tgl_dari_default' and '$tgl_sampai_default' group by kode_satker"));
        ...
        $realisasi_publikasi_total_persen = $satker_id;
        $realisasi_publikasi_total_angka = $satker_id;
        $total = $total;
        $total_persen = $total;
        $zona_satker_list_ii = $nama_satker;
        $zona_satker_list_ii_persen = $nama_satker;
        ...
        $total_medlok_humassatker = 0;
        $total_mednas_humaskanwil = 0;
        $total_mednas_humassatker = 0;
        $array_sum_medlok = [];
        if (auth()->user()->roles == "humas_kanwil") {
            ...
            //echo "<pre>"; print_r($get_media); die;
            ...
            $array_filterberita = [];
            foreach ($berita_humas as $ind_beritahumas => $val_beritahumas) {
                $medlok_humassatker = count(json_decode($val_beritahumas->media_lokal));
                $data_medlok_humassatker = json_decode($val_beritahumas->media_lokal);

                $mednas_humassatker = count(json_decode($val_beritahumas->media_nasional));
                $data_mednas_humassatker = json_decode($val_beritahumas->media_nasional);

                foreach ($data_medlok_humassatker as $in_medlokhumassatker => $val_medlokhumassatker) {
                    array_push($array_filterberita, (object)[
                        'kode_media' => explode("|||", $val_medlokhumassatker)[0]
                    ]);

                }
                $total_medlok_humassatker += $medlok_humassatker;
                $total_mednas_humassatker += $mednas_humassatker;
            }
